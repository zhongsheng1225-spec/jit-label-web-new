...
def load_mapping_excel(path: Path) -> dict[str, str]:
    wb = openpyxl.load_workbook(path, data_only=True, read_only=True)
    ws = _pick_mapping_sheet(wb)
    mapping: dict[str, str] = {}
    for code_raw, name_raw in ws.iter_rows(min_row=_mapping_start_row(ws), min_col=1, max_col=2, values_only=True):
...
def load_mapping_file(path: Path) -> dict[str, str]:
    if path.suffix.lower() == ".xlsx":
        return load_mapping_excel(path)
...
def _pick_mapping_sheet(workbook: openpyxl.Workbook):
